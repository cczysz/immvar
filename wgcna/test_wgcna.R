...
loadExpression <- function(celltype) {
	...
	female=expr[, phen$Sex=='Female']
	fem_race <- table(phen[phen$Sex=='Female', 'Race'])
	fem_subset <- c(sample(c(0,1), fem_race[1], replace=T), sample(c(0,1), fem_race[2], replace=T), sample(c(0,1), fem_race[3], replace=T))
	expr <- list(female1=t(female[, !!fem_subset]), female2=t(female[, !fem_subset]))
	return(expr)
}
...
for (i in seq(length(exp.list))) {
	...
	pdf.prefix <- paste(celltype, names(exp.list)[i], 'test', sep='_')
	print(pdf.prefix)
	expr <- exp.list[[i]]
