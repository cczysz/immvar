...
study="immvar.CD14 meta.all.cd14_immvar1.txt meta.all.cd14_all1.txt"
...
for i in $(echo $study); do
	for e in $(echo $sets); do
		awk 'NR>=2&&NR<=21' $i*$e*/gsea_report_for_na_pos_*xls | awk -F "\t" '{if ($7 < 0.05) {print $1}}' > $i.$e.na_pos.filter.nompval.0.05.txt ;
	done;
done;

for i in $(echo $study); do
	for e in $(echo $sets); do
		awk 'NR>=2&&NR<=21' $i*$e*/gsea_report_for_na_pos_*xls | awk -F "\t" '{if ($8 < 0.25) {print $1}}' > $i.$e.na_pos.filter.fdr.0.25.txt;
	done;
done;
