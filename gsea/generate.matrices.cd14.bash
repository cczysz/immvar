...
FDIR=/scratch/t.cczysz/gsea_out/CD14
...
cd14study="meta.all.cd14_all1.txt meta.all.cd14_immvar1.txt immvar.CD14"
#cd4study="meta.cd4_rep1.txt meta.all.cd4_weight1.txt immvar.CD4"
...
for i in $(echo $cd14study); do
	for e in $(echo $sets); do
		cat $i*$e*/gsea_report_for_na_pos_*xls | awk -F "\t" '{if ($7 < 0.05) {print $1}}' > $i.$e.na_pos.filter.nompval.0.05.txt ;
	done;
done;

for i in $(echo $cd14study); do
	for e in $(echo $sets); do
		cat $i*$e*/gsea_report_for_na_pos_*xls | awk -F "\t" '{if ($8 < 0.25) {print $1}}' > $i.$e.na_pos.filter.fdr.0.25.txt;
	done;
done;
