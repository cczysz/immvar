...
if (!file.exists('/group/stranger-lab/czysz/ImmVar/cd14.vst.towfique.Robj')) {
cd14.vst.list <- list()
for (i in seq(2)) {
for (j in (i+1):3) {
	vst <- pairwiseVst(eset[, colnames(eset)%in%cd14.pops[[i]]],  eset[, colnames(eset)%in%cd14.pops[[j]]])
	cd14.vst.list <- unlist(list(cd14.vst.list, list(vst)), recursive=FALSE)
	} } 
	save(cd14.vst.list,file='/group/stranger-lab/czysz/ImmVar/cd14.vst.towfique.Robj')
} else load('/group/stranger-lab/czysz/ImmVar/cd14.vst.towfique.Robj')
