...
if (!file.exists('/group/stranger-lab/czysz/ImmVar/vst.cd14.Robj')) {
cd14.vst.list <- list()
for (i in seq(2)) {
for (j in (i+1):3) {
	...
	save(cd14.vst.list,file='/group/stranger-lab/czysz/ImmVar/vst.cd14.Robj')
...
if (!file.exists('/group/stranger-lab/czysz/ImmVar/vst.cd4.Robj')) {
cd4.vst.list <- list()
for (i in seq(2)) {
for (j in (i+1):3) {
	vst <- pairwiseVst(exp.cd4[[i]], exp.cd4[[j]])
	cd4.vst.list <- unlist(list(cd4.vst.list, list(vst)), recursive=FALSE)
	} }
	save(cd4.vst.list,file='/group/stranger-lab/czysz/ImmVar/vst.cd4.Robj')
