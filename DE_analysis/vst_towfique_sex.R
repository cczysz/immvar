# Script to perform sex Vst analysis on GEO data generated by Towfique Raj et al.
...
#for (i in seq(2)) {
for (i in seq(3)) {
	vst <- pairwiseVst(eset[, colnames(eset)%in%(cd14.pops[[i]][phen.cd14$Race==pops[i] & phen.cd14$Sex=='Male'])], 
		eset[, colnames(eset)%in%(cd14.pops[[i]][phen.cd14$Race==pops[i] & phen.cd14$Sex=='Female'])])
