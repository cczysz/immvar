...
#for (f in c('cd4_meta_immvar1.txt', 'cd14_meta_immvar1.txt')) {
for (f in c('cd4_meta_immvar1.txt')) {
	...
	pvals <- data.frame(meta_res$Zscore, row.names=rownames(meta_res))
	meta_res$q.value <- p.adjust(meta_res$P.value, method='fdr')

	#meta_res <- subset(meta_res, q.value<0.05)
	...
	a.order <- a[order(a)]	
	print(head(a))
	to.write <- uni[substr(names(a[order(a)]),1,15), "hgnc_symbol"]
	#to.write.fem <- uni[substr(names(a[order(a)]),1,15), "hgnc_symbol"][a.order>0]
	#to.write.mal <- uni[substr(names(a[order(a)]),1,15), "hgnc_symbol"][a.order<0]
	#to.write <- cbind(uni[substr(names(a[order(a)]),1,15), "hgnc_symbol"], -log10(a[order(a)]))
	f.name <- paste('meta_webges_all', paste(unlist(strsplit(f, split='_'))[c(1,3)], collapse='_'), 'rnk', sep='.')
	write.table(to.write, file=paste('/home/t.cri.cczysz/', f.name, sep=''), quote=F, row.names=F, col.names=F, sep="\t")
	q('no')
	f.name <- paste('meta_webges_male', paste(unlist(strsplit(f, split='_'))[c(1,3)], collapse='_'), 'rnk', sep='.')
	write.table(to.write.mal, file=paste('/home/t.cri.cczysz/', f.name, sep=''), quote=F, row.names=F, col.names=F, sep="\t")
	f.name <- paste('meta_webges_female', paste(unlist(strsplit(f, split='_'))[c(1,3)], collapse='_'), 'rnk', sep='.')
	write.table(to.write.fem, file=paste('/home/t.cri.cczysz/', f.name, sep=''), quote=F, row.names=F, col.names=F, sep="\t")
}
}
